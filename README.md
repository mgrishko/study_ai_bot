...
### Меню (Reply Keyboard - кнопки внизу чата)

После /start пользователь видит удобное меню с кнопками:

**Для пользователей:**
```
🛍️ Каталог    | 📦 Мои заказы
📚 Категории   | ❓ Помощь
ℹ️ О магазине   | 🎨 AI
```
...
```
🛍️ Каталог      | 📦 Мои заказы
⚙️ Администратор | ❓ Помощь
🎨 AI
```

Каждая кнопка обрабатывается отдельным обработчиком.

### Для користувачів:
- `/start` - Початок роботи (показывает меню)
- `/help` - Список команд
...
- `/generate` - AI генератор изображений

### Text-to-Speech (TTS) озвучивание

Пользователи могут прослушивать описание товара:
- 🔊 Кнопка "Прослухати опис" при просмотре товара
- Поддержка языков: Українська, Русский, English и др.
- Автоматическое обрезание текста при необходимости
